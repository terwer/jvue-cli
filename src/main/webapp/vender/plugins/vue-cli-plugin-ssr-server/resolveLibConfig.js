...
module.exports = (api, { entry, name, formats }, options) => {
  ...
  function genConfig(format, postfix = format, genHTML) {
    ...
    if (!isVueEntry) {
      ...
      if (
        !/\b(export\s+default|export\s{[^}]+as\s+default)\b/.test(entryContent)
      ) {
  ...
  if (configs.indexOf(undefined) !== -1) {
    const unknownFormats = formatArray
      .filter(f => configMap[f] === undefined)
      .join(", ");
    abort(`Unknown library build formats: ${unknownFormats}`);
  }
